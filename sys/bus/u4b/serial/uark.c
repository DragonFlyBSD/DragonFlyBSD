...
DRIVER_MODULE(uark, uhub, uark_driver, uark_devclass, NULL, NULL);
...
uark_attach(device_t dev)
{
	...
	lockinit(&sc->sc_lock, "uark", 0, LK_CANRECURSE);
	...
	error = usbd_transfer_setup
	    ...
	    uark_xfer_config, UARK_N_TRANSFER, sc, &sc->sc_lock);
