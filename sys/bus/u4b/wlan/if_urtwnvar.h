...
#define URTWN_CMDQ_LOCK_INIT(sc)	lockinit(&(sc)->cmdq_mtx, "cmdq lock", 0, 0);
#define URTWN_CMDQ_LOCK(sc)		lockmgr(&(sc)->cmdq_mtx, LK_EXCLUSIVE)
#define URTWN_CMDQ_UNLOCK(sc)		lockmgr(&(sc)->cmdq_mtx, LK_RELEASE)
#define URTWN_CMDQ_LOCK_DESTROY(sc)	lockuninit(&(sc)->cmdq_mtx)

#define URTWN_NT_LOCK_INIT(sc)		lockinit(&(sc)->nt_mtx, "node table lock", 0, 0)
