...
#include <bus/u4b/wlan/if_urtwnvar.h>
    ...
    const uint8_t mac[IEEE80211_ADDR_LEN])
{
	...
	uvp = kmalloc(sizeof(struct urtwn_vap), M_80211_VAP, M_WAITOK | M_ZERO);
	vap = &uvp->vap;
...
urtwn_rx_copy_to_mbuf(struct urtwn_softc *sc, struct r92c_rx_stat *stat,
    int totlen)
{
	/* ic is not used, remove to fix gcc warning */
	/* struct ieee80211com *ic = &sc->sc_ic; */
	struct mbuf *m;
	uint32_t rxdw0;
	int pktlen;
...
urtwn_report_intr(struct usb_xfer *xfer, struct urtwn_data *data)
{
	...
	/* ic not used, remove to fix gcc error */
	/* struct ieee80211com *ic = &sc->sc_ic;*/
    ...
    const uint8_t mac[IEEE80211_ADDR_LEN])
{
	...
	un = kmalloc(sizeof (struct urtwn_node), M_80211_NODE,
		     M_INTWAIT | M_ZERO);
