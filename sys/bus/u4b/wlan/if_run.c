...
run_iter_func(void *arg, struct ieee80211_node *ni)
{
	...
	if (sc->rvp_cnt <= 1 && (vap->iv_opmode == IEEE80211_M_IBSS ||
		...
		IFNET_STAT_INC(ifp, oerrors, le16toh(sta[0].error.fail));
...
run_rt5592_set_chan(struct run_softc *sc, u_int chan)
{
	...
	if (chan <= 14) {
		...
		for (i = 0; i < NELEM(rt5592_2ghz_def_rf); i++) {
...
run_rt3593_rf_init(struct run_softc *sc)
{
	...
	/* Initialize RF registers to default value. */
	for (i = 0; i < NELEM(rt3593_def_rf); i++) {
...
run_rt5390_rf_init(struct run_softc *sc)
{
	...
	/* Initialize RF registers to default value. */
	if (sc->mac_ver == 0x5592) {
		for (i = 0; i < NELEM(rt5592_def_rf); i++) {
		...
		for (i = 0; i < NELEM(rt5392_def_rf); i++) {
		...
		for (i = 0; i < NELEM(rt5390_def_rf); i++) {
