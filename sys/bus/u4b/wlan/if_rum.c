/*	$FreeBSD: head/sys/dev/usb/wlan/if_rum.c 298895 2016-05-01 18:53:12Z avos $	*/
...
#include <bus/u4b/wlan/if_rumreg.h>
#include <bus/u4b/wlan/if_rumvar.h>
#include <bus/u4b/wlan/if_rumfw.h>
...
rum_attach(device_t self)
{
	...
	error = usbd_transfer_setup(uaa->device, &iface_index,
	    sc->sc_xfer, rum_config, RUM_N_TRANSFER, sc, &sc->sc_lock);
	if (error) {
    ...
    const uint8_t mac[IEEE80211_ADDR_LEN])
{
	...
	if (!TAILQ_EMPTY(&ic->ic_vaps))		/* only one at a time */
		return NULL;
	rvp = kmalloc(sizeof(struct rum_vap), M_80211_VAP, M_WAITOK | M_ZERO);
	vap = &rvp->vap;
