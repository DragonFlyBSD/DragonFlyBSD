...
#define WPI_TX_LOCK_INIT(_sc) \
	lockinit(&(_sc)->tx_mtx, "tx path lock", 0, 0)
...
#define WPI_NT_LOCK_INIT(_sc) \
	lockinit(&(_sc)->nt_mtx, "node table lock", 0, 0)
...
#define WPI_TXQ_LOCK_INIT(_sc) \
	lockinit(&(_sc)->txq_mtx, "txq/cmdq lock", 0, 0)
...
#define WPI_TXQ_STATE_LOCK_INIT(_sc) \
	lockinit(&(_sc)->txq_state_mtx, "txq state lock", 0, 0)
#define WPI_TXQ_STATE_LOCK(_sc)		lockmgr(&(_sc)->txq_state_mtx, LK_EXCLUSIVE)
