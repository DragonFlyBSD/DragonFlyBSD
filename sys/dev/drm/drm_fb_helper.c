...
int drm_fb_helper_add_one_connector(struct drm_fb_helper *fb_helper, struct drm_connector *connector)
{
	...
	if (fb_helper->connector_count + 1 > fb_helper->connector_info_alloc_count) {
		temp = krealloc(fb_helper->connector_info,
				sizeof(struct drm_fb_helper_connector *) * (fb_helper->connector_count + 1),
				M_DRM, GFP_KERNEL);
		if (!temp)
			return -ENOMEM;
			   ...
			   const struct drm_fb_helper_funcs *funcs)
{
	...
	lockinit(&helper->dirty_lock, "drm_fb_helper dirty_lock", 0, LK_CANRECURSE);
