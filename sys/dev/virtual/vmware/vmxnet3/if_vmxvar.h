 ...
 *
 * $FreeBSD: head/sys/dev/vmware/vmxnet3/if_vmxvar.h 313911 2017-02-18 06:18:14Z loos $
 */
...
#define VMXNET3_RXQ_LOCK(_rxq)		lockmgr(&(_rxq)->vxrxq_lock, LK_EXCLUSIVE)
#define VMXNET3_RXQ_UNLOCK(_rxq)	lockmgr(&(_rxq)->vxrxq_lock, LK_RELEASE)
#define VMXNET3_RXQ_LOCK_ASSERT(_rxq)		\
    KKASSERT(lockowned(&(_rxq)->vxrxq_lock) != 0)
...
#define VMXNET3_CORE_LOCK_INIT(_sc, _name) \
    lockinit(&(_sc)->vmx_lock, "VMXNET3 Lock", 0, 0)
#define VMXNET3_CORE_LOCK_DESTROY(_sc)	lockuninit(&(_sc)->vmx_lock)
#define VMXNET3_CORE_LOCK(_sc)		lockmgr(&(_sc)->vmx_lock, LK_EXCLUSIVE)
#define VMXNET3_CORE_UNLOCK(_sc)	lockmgr(&(_sc)->vmx_lock, LK_RELEASE)
#define VMXNET3_CORE_LOCK_ASSERT(_sc)	KKASSERT(lockowned(&(_sc)->vmx_lock) != 0)
#define VMXNET3_CORE_LOCK_ASSERT_NOTOWNED(_sc) \
    KKASSERT(lockowned(&(_sc)->vmx_lock) == 0)
