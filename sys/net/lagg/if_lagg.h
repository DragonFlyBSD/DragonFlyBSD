...
struct lagg_softc {
	...
	void	(*sc_start_dispatch)(netmsg_t msg);
...
#define	LAGG_LOCK_INIT(_sc)	lockinit(&(_sc)->sc_lock, "if_lagg rmlock", 0, LK_CANRECURSE)
#define	LAGG_LOCK_DESTROY(_sc)	lockuninit(&(_sc)->sc_lock)
#define	LAGG_RLOCK(_sc)		lockmgr(&(_sc)->sc_lock, LK_SHARED)
#define	LAGG_WLOCK(_sc)		lockmgr(&(_sc)->sc_lock, LK_EXCLUSIVE)
#define	LAGG_RUNLOCK(_sc)	lockmgr(&(_sc)->sc_lock, LK_RELEASE)
...
#define	LAGG_CALLOUT_LOCK_INIT(_sc)					\
	    lockinit(&(_sc)->sc_call_lock, "if_lagg callout mutex", 0, 0)
#define	LAGG_CALLOUT_LOCK_DESTROY(_sc)	lockuninit(&(_sc)->sc_call_lock)
