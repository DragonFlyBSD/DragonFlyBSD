 ...
 * $DragonFly: src/lib/libc/rpc/clnt_unix.c,v 1.5 2005/11/13 12:27:04 swildner Exp $
 */
 ...
 */
static int
readunix(struct ct_data *ct, caddr_t buf, int len)
{
	fd_set *fds, readfds;
...
static int
writeunix(struct ct_data *ct, caddr_t buf, int len)
{
	int i, cnt;
