...
noone: 				write(2, "Can't find your logname.  Who Are You?\n", 39);
	...
	if ((lpbuf == NULL) || (inbuffer == NULL))	/* malloc() failure */
		died(-285);
	...
	while (1) {
		if (dropflag == 0)	/* see if there is an object here */
			lookforobject();
		else			/* don't show it just dropped an item */
			dropflag = 0;
		if (hitflag == 0) {
			if (c[HASTEMONST])
				movemonst();
...
showstr(void)
{
	...
	for (number = 3, i = 0; i < 26; i++)
		if (iven[i])	/* count items in inventory */
			number++;
	t_setup(number);
	qshowstr();
	t_endup(number);
...
randmonst(void)
{
	if (c[TIMESTOP])	/* don't make monsters if time is stopped */
		return;
	if (--rmst <= 0) {
		rmst = 120 - (level << 2);
		fillmonst(makemonst(level));
