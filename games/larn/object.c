...
{
	int i, j;
	if (c[TIMESTOP])	/* can't find objects is time is stopped */
		return;
	...
	case OTRAPARROWIV:
		if (rnd(17) < 13)	/* for an arrow trap */
			return;
...
{
	int i, j, k;
	if (pot < 0 || pot >= MAXPOTION)	/* check for within bounds */
		return;
...
{
	int i, j;
	if (typ < 0 || typ >= MAXSCROLL)	/* be sure we are within bounds */
		return;
	...
	case 18:
		for (i = 0; i < 11; i++)
			c[(int)exten[i]] <<= 1;	/* spell extension */
		break;
