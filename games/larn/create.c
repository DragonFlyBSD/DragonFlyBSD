...
makeplayer(void)
{
	...
	cbak[SPELLS] = -50;
	for (i = 0; i < 6; i++)	/* make the attributes, ie str, int, etc. */
		c[i] = 12;
	recalc();
}
...
{
	int i, j;
	if (beenhere[(int)level])	/* put the level back into storage */
		savelevel();
...
makemaze(int k)
{
	...
	if (k > 1 && (rnd(17) <= 4 || k == MAXLEVEL - 1 || k == MAXLEVEL + MAXVLEVEL - 1))
		if (cannedlevel(k))	/* read maze from data file */
			return;
	if (k == 0)
		tmp = 0;
	else
...
makeobject(int j)
{
	...
	if (j == MAXLEVEL)  /* volcano shaft up from the temple */
		fillroom(OVOLUP, 0);
