...
draws(int xmin, int xmax, int ymin, int ymax)
{
	...
	d_xmin = xmin;	/* for limited screen drawing */
	d_xmax = xmax;
	d_ymin = ymin;
	d_ymax = ymax;
	drawscreen();
...
drawscreen(void)
{
	...
	for (i = d_ymin; i < d_ymax; i++)
		for (j = d_xmin; j < d_xmax; j++)
			if (know[j][i] == 0)
				screen[j][i] = ' ';
			else if ((l = mitem[j][i]) != 0)
				screen[j][i] = monstnamelist[l];
...
{
	int l, n, i, j;
	if (c[CONFUSE])		/*if confused any dir */
		if (c[LEVEL] < rnd(30))
			dir = rund(9);
	l = playerx + diroffx[dir];
	n = playery + diroffy[dir];
