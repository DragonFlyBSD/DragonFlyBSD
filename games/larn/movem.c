...
{
	int i, j;
	if (c[TIMESTOP])	/* no action if time is stopped */
		return;
	if (c[HASTESELF])
		if ((c[HASTESELF] & 1) == 0)
			return;
	if (spheres)		/* move the spheres of annihilation if any */
		movsphere();
	if (c[HOLDMONST])	/* no action if monsters are held */
		return;
...
movsphere(void)
{
	...
	if (x)		/* if any spheres, point to them */
		sp = sph;
	else		/* no spheres */
		return;
