...
paytaxes(long x)
{
	...
	for (i = 0; i < SCORESIZE; i++)
		if (winr[i].suid == userid)	/* look for players winning entry */
			...
			if (winr[i].score > 0) {
				amt = winr[i].taxes;
				if (x < amt)	/* don't overpay taxes (Ughhhhh) */
					amt = x;
				winr[i].taxes -= amt;
...
newscore(long score, char *whoo, int whyded, int winner)
{
	int i;
	long taxes;
	if (readboard() < 0)	/* do the scoreboard */
		return;
	/* if a winner then delete all non-winning scores */
	if (cheat)		/* if he cheated, don't let him win */
		winner = 0;
	if (winner) {
		for (i = 0; i < SCORESIZE; i++)
...
invalid:
	...
	lflush();
	f = 0;
	if (ckpflag)	/* remove checkpoint file if used */
		unlink(ckpfile);
	if (x < 0) {
		f++;
		x = -x;
	}	/* if we are not to display the scores */
	if ((x == 300) || (x == 257))	/* for quick exit or saved game */
		exit(0);
	if (x == 263)
		win = 1;
	else
	...
	if ((x == 256) || (x == 257) || (f != 0))
		exit(0);
	if (scorerror == 0)	/* if we updated the scoreboard */
		showscores();
	if (x == 263)
		mailbill();
	exit(0);
...
getplid(char *nam)
{
	...
	for (;;) {	/* now search for the name in the player id file */
		...
		if (p2 == NULL)	/* EOF? */
			break;
		if (no > high)	/* accumulate highest id # */
			high = no;
