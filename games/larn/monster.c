...
cast(void)
{
	...
	if (a == '\33')	/* to escape casting a spell */
		goto over;
	if ((b = getchr()) == '\33')	/* to escape casting a spell */
		goto over;
	if ((d = getchr()) == '\33') {
over:		lprcat(aborted);
		c[SPELLS]++;
...
/* ----- LEVEL 4 SPELLS ----- */
	...
	case 24:	/* globe of invulnerability */
		if (c[GLOBE] == 0)
			c[MOREDEFENSES] += 10;
		c[GLOBE] += 200;
		loseint();
...
/* ----- LEVEL 6 SPELLS ----- */
	...
	case 32:	/* sphere of annihilation */
...
fullhit(int xx)
{
	...
	if (xx < 0 || xx > 20)	/* fullhits are out of range */
		return (0);
	if (c[LANCEDEATH])	/* lance of death */
		return (10000);
...
tdirect(int spnum)
{
	int x, y;
	int m;
	if (spnum < 0 || spnum >= SPNUM)	/* bad args */
		return;
	if (isconfuse())
		return;
	dirsub(&x, &y);
...
dirpoly(int spnum)
{
	...
	if (spnum < 0 || spnum >= SPNUM)	/* bad args */
		return;
	if (isconfuse())	/* if he is confused, he can't aim his magic */
		return;
	dirsub(&x, &y);
	if (mitem[x][y] == 0) {
		lprcat("  There wasn't anything there!");
...
hitmonster(int x, int y)
{
	...
	if (c[TIMESTOP])	/* not if time stopped */
		return;
...
hitm(int x, int y, int amt)
{
	...
	monst = mitem[x][y];
	if (c[HALFDAM])		/* if half damage curse adjust damage points */
		amt >>= 1;
	if (amt <= 0)
		amt2 = amt = 1;
	lasthx = x;
...
something(int lvl)
{
	int j;
	int i;
	if (lvl < 0 || lvl > MAXLEVEL + MAXVLEVEL)	/* correct level? */
		return;
	if (rnd(101) < 8)	/* possibly more than one item */
		something(lvl);
	j = newobject(lvl, &i);
	createitem(j, i);
}
...
{
	int tmp = 32, j;
	if (level < 0 || level > MAXLEVEL + MAXVLEVEL)	/* correct level? */
		return (0);
	if (lev > 6)
		tmp = 37;
	else if (lev > 4)
...
spout2:		if (p) {
	...
	case 8:
		if (c[NOTHEFT])		/* he has a device of no theft */
			return (0);
		if (c[GOLD]) {
	...
	case 14:
		if (c[NOTHEFT])		/* he has device of no theft */
			return (0);
		if (emptyhanded() == 1) {
...
boom:		sphboom(x, y);		/* blow up stuff around sphere */
	...
	sp->lifetime = life;
	sp->p = 0;
	if (spheres == 0)	/* if first node in the sphere list */
		spheres = sp;
