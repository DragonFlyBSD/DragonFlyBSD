#!/usr/local/www/cgi-bin/tablecg
#
# $DragonFly: site/data/community/Attic/donations.cgi,v 1.1 2006/04/05 03:05:15 justin Exp $

$TITLE(DragonFly - Donations)

<h1>DragonFly - Donations</h1>

<p>
DragonFly is not yet incorporated as a non-profit, meaning no tax benefit is 
available for donating time or goods to the project.  However, developers can 
still use your contributed hardware.  
</p>

<p>
Please email donations at dragonflybsd.org to make a request or when a request is filled, so 
that we may update this page.  Requestors on this page should have at least one 
accepted contribution to DragonFly.
</p>

<p>
Donors, please contact the requestor directly (and CC: 
donations at dragonflybsd.org).  You and the donating/receiving party must 
independently resolve any delivery issues.  The DragonFly BSD Project is 
not responsible for any transactions.  
</p>

<p>
Some tips on sending donated equipment through the mail: Mailing materials out of 
the country usually requires some sort of customs declaration.  State clearly 
on any customs form that the equipment is used, is a gift to an individual and 
not a business, and the value of the equipment (usually less than $50 USD).  
Adding www.dragonflybsd.org as a reference can also help, as this provides 
additional information for a curious customs officer.  It also may help to 
repeat that information in a letter inside the packaging, in case it is opened 
or damaged.
</p>

<table width="100%" cellpadding="2" cellspacing="0" border="1">
<tr bgcolor="#ffcc00">
	<th>Requestor</th>
	<th>Requested Items</th>
	<th>Used for</th>
</tr>

<!-- use this as a template -->

<!-- posted dd/mm/yyyy -->
<!--
<tr>
	<td> 
		Joe Schmoe<br />
		joe at schmoe dot com<br />
		California, USA	
	</td>
	<td>
	Orbitech, Onitron, or Obzug Electronics SCSI drive, any size.
	</td>
	<td>
	SCSI controller support
	</td>
</tr>
-->

<!-- posted 27/07/2004 -->
<tr>
        <td>
                Emiel Kollof<br />
                coolvibe at hackerheaven dot org<br />
                Rijswijk, The Netherlands
        </td>
        <td>
        NVIDIA graphics hardware
        </td>
        <td>
        NVIDIA binary driver prerelease testing
        </td>
</tr>

<!-- posted 27/07/2004 -->
<tr>
        <td>
                Emiel Kollof<br />
                coolvibe at hackerheaven dot org<br />
                Rijswijk, The Netherlands
        </td>
        <td>
        SMP motherboard/processors, with an AGP slot
        </td>
        <td>
        NVIDIA binary driver prerelease testing
        </td>
</tr>

<!-- posted 27/07/2004 -->
<tr>
        <td>
                Emiel Kollof<br />
                coolvibe at hackerheaven dot org<br />
                Rijswijk, The Netherlands
        </td>
        <td>
        Hard drive >80 gig
        </td>
        <td>
        Source tree storage
        </td>
</tr>

<!-- posted 12/08/2004 -->
<tr>
        <td>
                Robert Garrett<br />
                rgarrett24 at cox dot net<br />
		Oklahoma City, Oklahoma
                
        </td>
        <td>
        x86 laptop with DragonFly support and enough disk space for build/makeworld
        </td>
        <td>
	DragonFly work while on the road
        </td>
</tr>

<!-- posted 03/13/2005 -->
<tr>
	<td>
		Simon 'corecode' Schubert<br />
		corecode at fs dot ei dot tum dot de<br />
		Munich, Germany
		
	</td>
	<td>
	Contemporary hardware and/in 19" case
	</td>
	<td>
	ISO building, additional community services
	</td>
</tr>

<!-- posted 03/13/2005 -->
<tr>
	<td>
		Simon 'corecode' Schubert<br />
		corecode at fs dot ei dot tum dot de<br />
		Munich, Germany
		
	</td>
	<td>
	2x 256MB (or larger) DDR RAM
	</td>
	<td>
	DragonFly development, Community services development
	</td>
</tr>

<!-- posted 03/13/2005 -->
<tr>
	<td>
		Simon 'corecode' Schubert<br />
		corecode at fs dot ei dot tum dot de<br />
		Munich, Germany
		
	</td>
	<td>
	16A IEC 320 C19 to Schuko Power Cord
	</td>
	<td>
	Running 4-way XEON for MP development
	</td>
</tr>

<!-- posted 03/10/2005 -->
<tr>
	<td>
		Jeffrey Hsu<br />
		hsu at dragonflybsd dot org
		San Jose, California
	</td>
	<td>
		Rack of client machines, one server, and gigE switch.
	</td>
	<td>
		Network stress testing, performance tuning, and benchmarking.
	</td>
</tr>

<!-- posted 03/10/2005 -->
<tr>
	<td>
		Jeffrey Hsu<br />
		hsu at dragonflybsd dot org
		San Jose, California
	</td>
	<td>
		two 10gigE NICs, one 10gigE switch, two PCI-Express machines
	</td>
	<td>
		High-speed networking
	</td>
</tr>

<!-- posted 03/10/2005 -->
<tr>
	<td>
		Jeffrey Hsu<br />
		hsu at dragonflybsd dot org
		San Jose, California
	</td>
	<td>
		Amazon wish list for hsu at dragonflybsd dot org
	</td>
	<td>
		reading
	</td>
</tr>

<!-- posted 03/10/2005 -->
<tr>
	<td>
		Jeffrey Hsu<br />
		hsu at dragonflybsd dot org
		San Jose, California
	</td>
	<td>
		CSRG code respository from http://www.mckusick.com/csrg/index.html
	</td>
	<td>
		looking up orignal reason for historical code changes
	</td>
</tr>

<!-- posted 03/10/2005 -->
<tr>
	<td>
		Jeffrey Hsu<br />
		hsu at dragonflybsd dot org
		San Jose, California
	</td>
	<td>
		8-way or higher multiprocessor machine
	</td>
	<td>
		improving MP scalability
	</td>
</tr>

</table>
