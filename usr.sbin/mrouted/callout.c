...
age_callout_queue(int elapsed_time)
{
	...
	if (ptr->time > elapsed_time) {
	    ...
	    dolog(LOG_DEBUG, 0, "about to call timeout %d (#%d)", ptr->id, i);
	    if (ptr->func)
		ptr->func(ptr->data);
	    free(ptr);
	}
...
timer_nextTimer(void)
{
    if (Q) {
	if (Q->time < 0) {
	    dolog(LOG_WARNING, 0, "timer_nextTimer top of queue says %d",
			Q->time);
	    return 0;
	}
	return Q->time;
...
timer_setTimer(int delay, cfunc_t action, void *data)
{
	...
	while (ptr) {
		...
		print_Q();
		IF_DEBUG(DEBUG_TIMEOUT)
		dolog(LOG_DEBUG, 0, "created timeout %d (#%d)", node->id, i);
		return node->id;
	    } else  {
		/* keep moving */
...
timer_clearTimer(int timer_id)
{
	...
	if (ptr->id == timer_id) {
		...
		free(ptr->data);
	    IF_DEBUG(DEBUG_TIMEOUT)
	    dolog(LOG_DEBUG, 0, "deleted timer %d (#%d)", ptr->id, i);
	    free(ptr);
	    print_Q();
	    return 1;
	}
    ...
    IF_DEBUG(DEBUG_TIMEOUT)
    dolog(LOG_DEBUG, 0, "failed to delete timer %d (#%d)", timer_id, i);
    print_Q();
    return 0;
}
...
print_Q(void)
{
    ...
    IF_DEBUG(DEBUG_TIMEOUT)
	for (ptr = Q; ptr; ptr = ptr->next)
	    dolog(LOG_DEBUG, 0, "(%d,%d) ", ptr->id, ptr->time);
}
#endif /* IGMP_DEBUG */
