    ...
    for (;;)
        ...
        case HEAD:
            ...
            if (((BITS(8) << 8) + (hold >> 8)) % 31) {
                strm->msg = (const char *)"incorrect header check";
                state->mode = BAD;
                break;
            }
            if (BITS(4) != Z_DEFLATED) {
                strm->msg = (const char *)"unknown compression method";
                state->mode = BAD;
                break;
            }
            DROPBITS(4);
        ...
        case TYPEDO:
            ...
            case 2:                             /* dynamic block */
                ...
                break;
            case 3:
                strm->msg = (const char *)"invalid block type";
                state->mode = BAD;
            }
            DROPBITS(2);
            break;
        case STORED:
            ...
            if ((hold & 0xffff) != ((hold >> 16) ^ 0xffff)) {
                strm->msg = (const char *)"invalid stored block lengths";
                state->mode = BAD;
                break;
            }
        ...
        case TABLE:
            ...
            if (state->nlen > 286 || state->ndist > 30) {
                strm->msg = (const char *)"too many length or distance symbols";
                state->mode = BAD;
                break;
            }
#endif
        ...
        case LENLENS:
            ...
            if (ret) {
                strm->msg = (const char *)"invalid code lengths set";
                state->mode = BAD;
                break;
            }
        ...
        case CODELENS:
            while (state->have < state->nlen + state->ndist) {
                ...
                if (here.val < 16) {
                    ...
                    if (here.val == 16) {
                        ...
                        DROPBITS(here.bits);
                        if (state->have == 0) {
                            strm->msg = (const char *)"invalid bit length repeat";
                            state->mode = BAD;
                            break;
                        }
            ...
            ret = inflate_table(LENS, state->lens, state->nlen, &(state->next),
                                &(state->lenbits), state->work);
            if (ret) {
                strm->msg = (const char *)"invalid literal/lengths set";
                state->mode = BAD;
                break;
            }
            ...
            if (ret) {
                strm->msg = (const char *)"invalid distances set";
                state->mode = BAD;
                break;
            }
        ...
        case CHECK:
            if (state->wrap) {
                ...
                if ((ZSWAP32(hold)) != state->check) {
                    strm->msg = (const char *)"incorrect data check";
                    state->mode = BAD;
                    break;
                }
                INITBITS();
