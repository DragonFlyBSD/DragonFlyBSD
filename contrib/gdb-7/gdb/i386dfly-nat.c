    ...
    ptid_t ptid, int step, enum gdb_signal signal)
{
	...
	if (pid == -1)
		 ...
		 * non-threaded case, where "resume all threads" and "resume
		 * inferior_ptid" are the same.  */
	...
	if (!step) {
		 ...
		 * as SIGALRM) and we do a PT_STEP, the process never really
		 * has a chance to run because the kernel needs to notify the
		 * debugger that a signal is being sent.  Therefore, the
		 * process never goes through the kernel's trap() function
		 * which would normally clear it.  */
...
i386dfly_supply_pcb(struct regcache *regcache, struct pcb *pcb)
{
	 ...
	 * 
	 * The pcb contains %eip, %ebx, %esp, %ebp, %esi, %edi and %gs. This
	 * accounts for all callee-saved registers specified by the psABI and
	 * then some.  Here %esp contains the stack pointer at the point just
	 * after the call to cpu_switch().  From this information we
